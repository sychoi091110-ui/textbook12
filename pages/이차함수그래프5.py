...
# 사용자 입력
quiz = st.radio("정답 선택", ["A", "B", "C", "D"], index=None)

# 제출 버튼
submit = st.button("정답 제출")
...
if submit:
    if quiz is None:
        ...
        if quiz == "A":
            st.error("❌ 오답입니다. a가 커질수록 그래프는 **x축에 가까워집니다.**")
        elif quiz == "B":
            st.success("✅ 정답입니다! 아주 잘 이해했습니다.")
        elif quiz == "C":
            st.error("❌ 오답입니다. |a|가 작을수록 그래프가 퍼집니다.")
        elif quiz == "D":
